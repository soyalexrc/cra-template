...
import {
  ...
  IconButton,
  useMediaQuery, Paper,
  Typography, Grid, TextField, InputAdornment, Divider, Select, MenuItem, FormControl
} from '@mui/material';
...
export default function ComissionModal({setOpen, open, data}) {
  const largeScreen = useMediaQuery((theme) => theme.breakpoints.up('md'));
  const {getUser} = useAuth()
  const {updateSellerAndCommission, loading} = useProperties()
    ...
    >
      <Paper elevation={2} sx={{display: 'flex', justifyContent: 'center', padding: '1rem 3rem'}}>
        <Box>
          ...
          {data && <Typography align='center' fontWeight='bold' style={{color: 'gray'}}>{data.code}</Typography>}
        </Box>
      ...
      {data &&
        <>
          <DialogContent sx={{padding: '1rem 3rem'}}>
            ...
            <Typography variat='caption' sx={{mb: 3}}>Seleccione la comision asociada al inmueble</Typography>
