...
import empty from '../../assets/img/workers-empty.png'
    ...
    <RootStyle title='Trabajadores | Alishop'>
      {providers.length < 1 && <EmptyProviders largeScreen={largeScreen}/>}
      {
        ...
        <>
          ...
          <Paper sx={{width: '100%', display: 'flex', justifyContent: 'center', p: 2}}>
      ...
      <BottomMenu
        actions={[{label: 'Nuevo trabajador', fn: () => navigate('registrar'), main: true, disabled: false}]}/>
      <BottomDrawer open={openDrawer} toggleDrawer={() => toggleDrawer()} largeScreen={largeScreen}/>
    </RootStyle>
  )
}
...
function BottomDrawer({open, toggleDrawer, largeScreen}) {
  return (
    <Drawer
      anchor='bottom'
      open={open}
    ...
    >
      <Box height={largeScreen ? 400 : 600}>
        <Typography variant='h6' align='center' sx={{ padding: '2rem 0' }}>Filtrar por perfil</Typography>
        <Divider sx={{ borderWidth: '1px' }} />
        <Container maxWidth='md' sx={{py: 5 }}>
          <Grid container spacing={2}>
            {
              ['Vendedor', 'Bodeguero', 'Packing', 'Transportista', 'Administrador'].map(rol => (
                <Grid key={rol} item xs={12} sm={6} md={4}>
                  <Box display='flex' justifyContent={largeScreen ? 'center' : 'left'}>
                    <FormControlLabel
                      control={<Checkbox />}
                      label={<Typography variant="body1">{rol}</Typography>}
                    />
                  </Box>
                </Grid>
              ))
        ...
        <Box display='flex' justifyContent='center' py={5}>
          <Button variant='contained' onClick={toggleDrawer}> Filtrar</Button>
        </Box>
      </Box>
    </Drawer>
  )
